...
namespace Maestro\Extension\Task\Command;
...
class DebugTaskCommand extends Command
{
    ...
    private function showTask(OutputInterface $output, $taskAlias): void
    {
        ...
        $output->writeln(sprintf('<comment>Task</>: %s', $definition->taskClass()));
        $output->writeln(sprintf('<comment>Handler</>: %s', $definition->serviceId()));
